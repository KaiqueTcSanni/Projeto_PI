...
# 1. TABELA DE PRODUTOS (Sua model original)
...
# 2. TABELA DE PEDIDOS (O "Pai" da venda)
class Pedido(models.Model):
    ...
    # Status para você controlar no Admin
    STATUS_CHOICES = [
        ('Pendente', 'Pendente'),
        ('Pago', 'Pago'),
        ('Cancelado', 'Cancelado'),
...
# 3. TABELA DE ITENS DO PEDIDO (Os "Filhos" - cada peça do PC)
class ItemPedido(models.Model):
    id_item = models.AutoField(primary_key=True)
    pedido = models.ForeignKey(Pedido, related_name='itens', on_delete=models.CASCADE)
    
    # Guardamos o nome e preço como texto/decimal fixo para histórico 
    # (mesmo que o produto original seja deletado ou mude de preço)
